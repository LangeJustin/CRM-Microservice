...
@ResponseStatus(code = PRECONDITION_FAILED,
                reason = "Der Kunde wurde zwischenzeitlich aktualisiert")
class ConcurrentUpdatedException extends RuntimeException {
    ConcurrentUpdatedException() {
        super();
    }

    ConcurrentUpdatedException(Throwable cause) {
        super(cause);
    }
}
